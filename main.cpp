...
int main() {
	...
	if (mood == 1) {
		while (end)
		{
			...
			printf("请输入密钥：");
			for (int i = 0; i < 16; i++) scanf("%I8u", &KEY[i]); //0x2b 0x7e 0x15 0x16 0x28 0xae 0xd2 0xa6 0xab 0xf7 0x15 0x88 0x09 0xcf 0x4f 0x3c
			printf("请输入明文：");
			for (int i = 0; i < 16; i++) scanf("%I8u", &WORDS[i]); // 0x32 0x43 0xf6 0xa8 0x88 0x5a 0x30 0x8d 0x31 0x31 0x98 0xa2 0xe0 0x37 0x07 0x34
			aesEncrypt(KEY, 16, WORDS, ct, 16); // 加密
			printHex((uint8_t*)WORDS, 16, "明文是:"); // 打印初始明文数据

			printHex(ct, 16, "加密后的密文是:");  // 打印加密后的密文

			aesDecrypt(KEY, 16, ct, plain, 16);       // 解密
			printHex(plain, 16, "密文解密得到:"); // 打印解密后的明文数据
			printf("\n");
			printf("是否继续加密？y是  n否: ");
			scanf("%c", &c);
			if (c == 'y') end = false;
	...
	else if (mood == 2)
	{
		...
		{
			char KEY[16];
			uint8_t KEY2[] = "1234567890123456";
			uint8_t *WORDS2 = (uint8_t*)"abcdefghijklmnopqrstuvwxyz123456";
			char WORDS[10000];
			uint8_t ct2[32] = { 0 };
			uint8_t plain2[32] = { 0 };
			char c;
			...
			printf("请输入密钥：");
			for (int i = 0; i < 16; i++) scanf("%c", &KEY[i]); //1234567890123456
			printf("请输入明文：");
			for (int i = 0; i < 16; i++) scanf("%c", &WORDS[i]); //abcdefghijklmnopqrstuvwxyz123456

			//加密32字节明文
			aesEncrypt(KEY2, 16, WORDS2, ct2, 32);

			printf("\n明文是:\n%s\n", WORDS2);
			printHex(ct2, 32, "加密后的密文是:");

			// 解密32字节密文
			aesDecrypt(WORDS2, 16, ct2, plain2, 32);
			// 打印16进制形式的解密后的明文
			printHex(plain2, 32, "密文解密后得到:");
